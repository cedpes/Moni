    ...
    <div className="min-h-screen bg-[var(--bg-app)] pb-24">
      <header className="sticky top-0 z-10 bg-[var(--header-blur-bg)] backdrop-blur-xl border-b border-[var(--border-default)] px-5 pt-14 pb-3">
        <div className="flex items-start justify-between">
          <h1 className="text-[28px] font-bold tracking-tight text-[var(--text-primary)] leading-tight">Épargne</h1>
        </div>
      </header>
      ...
      {loading ? (
        ...
        <div className="px-4 pt-10">
          <div className="bg-[var(--bg-surface)] rounded-[20px] px-4 py-8 text-center">
            <p className="text-[32px] mb-3">⚠️</p>
            <p className="text-[15px] font-medium text-[var(--text-primary)]">Collection manquante</p>
            <p className="text-[13px] text-[var(--text-secondary)] mt-1">{error}</p>
          </div>
        </div>
      ) : (
        <div className="px-4 pt-5 space-y-4">
          <div className="bg-[var(--bg-surface)] rounded-[20px] p-5">
            <div className="flex items-end justify-between mb-4">
              <div>
                <p className="text-[13px] text-[var(--text-secondary)] mb-1">Total épargné</p>
                <p className="text-[26px] font-bold text-[var(--text-primary)] tracking-tight">{fmt(total)}</p>
              </div>
              <div className="text-right">
                <p className="text-[13px] text-[var(--text-secondary)] mb-1">Reste à épargner</p>
          ...
          <p className="text-[12px] font-semibold tracking-widest uppercase text-[var(--text-secondary)] px-1">Objectifs d&apos;épargne</p>

          {goals.length === 0 ? (
            <div className="bg-[var(--bg-surface)] rounded-[20px] px-4 py-10 text-center">
              <p className="text-[32px] mb-3">💰</p>
              <p className="text-[15px] font-medium text-[var(--text-primary)]">Aucun objectif</p>
              <p className="text-[13px] text-[var(--text-secondary)] mt-1">Appuie sur + pour en créer un</p>
            </div>
          ) : (
            <div className="space-y-3">
              {goals.map(g => {
                ...
                return (
                  <div key={g.id} className="bg-[var(--bg-surface)] rounded-[16px] p-4">
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-11 h-11 rounded-full flex items-center justify-center text-xl flex-shrink-0" style={{ background: g.color ?? 'var(--bg-surface-2)' }}>
                        {g.icon}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[15px] font-semibold text-[var(--text-primary)]">{g.name}</p>
                        <p className="text-[12px] text-[var(--text-secondary)]">Objectif: {fmt(g.target_amount)} · Total épargné: {fmt(g.current_amount)}</p>
                        {g.target_date && (
                          <p className={`text-[12px] mt-0.5 ${overdue ? 'text-[#f87171]' : 'text-[var(--text-secondary)]'}`}>
                            {overdue ? 'Échéance dépassée · ' : 'Échéance : '}{fmtDate(g.target_date)}
                          </p>
                        )}
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <button onClick={() => openEdit(g)} className="w-7 h-7 rounded-full bg-[var(--bg-surface-2)] flex items-center justify-center">
                          <Pencil size={11} color="var(--text-secondary)" />
                        </button>
                        <button onClick={() => deleteGoal(g.id)} className="w-7 h-7 rounded-full bg-[var(--bg-surface-2)] flex items-center justify-center">
                          <X size={11} color="var(--text-secondary)" />
                        </button>
                      </div>
                    </div>
                    <div className="h-1.5 bg-[var(--bg-surface-2)] rounded-full overflow-hidden mb-3">
                      <div className="h-full rounded-full bg-[#fbbf24] transition-all" style={{ width: `${pct}%` }} />
                    </div>
                    <div className="flex items-center justify-between">
                      <p className="text-[12px] text-[var(--text-secondary)]">{pct}% atteint</p>
        ...
        <div className="fixed inset-0 bg-[var(--overlay)] z-[60] flex items-end justify-center"
          ...
          <div className="bg-[var(--bg-surface)] rounded-t-[24px] w-full max-w-lg p-5 pb-10">
            ...
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-[18px] font-bold text-[var(--text-primary)]">{editItem ? 'Modifier' : 'Nouvel objectif'}</h2>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="text-[13px] text-[var(--text-secondary)] block mb-1.5">Objectif (€)</label>
              ...
              <div>
                <label className="text-[13px] text-[var(--text-secondary)] block mb-1.5">Date de fin (optionnel)</label>
                <input type="date"
                  className="w-full h-11 border border-[var(--border-default)] rounded-[12px] px-3.5 text-[16px] bg-[var(--bg-surface-2)] text-[var(--text-primary)] outline-none focus:border-[#3b82f6]"
      ...
      {contribFor && (
        <div className="fixed inset-0 bg-[var(--overlay)] z-[60] flex items-end justify-center"
          ...
          <div className="bg-[var(--bg-surface)] rounded-t-[24px] w-full max-w-lg p-5 pb-10">
            <div className="w-9 h-1 bg-[var(--bg-surface-3)] rounded-full mx-auto mb-5" />
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-[18px] font-bold text-[var(--text-primary)]">Ajouter à &quot;{contribFor.name}&quot;</h2>
              <button onClick={() => setContribFor(null)} className="w-7 h-7 rounded-full bg-[var(--bg-surface-2)] flex items-center justify-center">
                <X size={14} color="var(--text-secondary)" />
              </button>
            </div>
            <div className="space-y-3">
              ...
              <button onClick={addContribution} disabled={saving || !contribAmount}
                className="w-full h-12 bg-[#fbbf24] text-[var(--bg-surface)] rounded-[14px] font-semibold text-[15px] flex items-center justify-center gap-2 mt-1 disabled:opacity-50 active:scale-[0.98] transition-all">
                {saving && <Loader2 size={16} className="animate-spin" />}
                Ajouter
              </button>
            </div>
