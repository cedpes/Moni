    ...
    <div className="min-h-screen bg-[#f5f5f7] pb-24">
      ...
      <header className="sticky top-0 z-10 bg-[#f5f5f7]/90 backdrop-blur-xl border-b border-black/[0.06] px-5 pt-14 pb-3">
        <div className="flex items-start justify-between mb-3">
          <div>
            <h1 className="text-[28px] font-bold tracking-tight text-[#1d1d1f] leading-tight">Calendrier</h1>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
          <span className="flex-shrink-0 px-3 py-1 rounded-full text-[12px] font-medium bg-[#fef0f0] text-[#c0392b]">
            Charges {fmt(totalCharges)}
          </span>
          <span className="flex-shrink-0 px-3 py-1 rounded-full text-[12px] font-medium bg-[#e8faf0] text-[#1a7f3c]">
            Revenus {fmt(totalIncome)}
          </span>
          {paidCharges > 0 && (
            <span className="flex-shrink-0 px-3 py-1 rounded-full text-[12px] font-medium bg-[#f2f2f7] text-[#86868b]">
              ✓ {fmt(paidCharges)} payé
            </span>
          )}
          {lateCharges.length > 0 && (
            <span className="flex-shrink-0 px-3 py-1 rounded-full text-[12px] font-medium bg-[#fff5e6] text-[#b45309]">
      ...
      {loading ? (
        ...
        <div className="px-4 pt-4 space-y-3">
          ...
          <div className="bg-white rounded-[20px] p-3">
            <div className="grid grid-cols-7 mb-1">
              {['L','M','M','J','V','S','D'].map((d, i) => (
                <div key={i} className={`text-center text-[11px] font-semibold py-1 ${i >= 5 ? 'text-[#ff3b30]' : 'text-[#86868b]'}`}>{d}</div>
              ))}
            </div>
              ...
              {Array(daysInMonth).fill(null).map((_, i) => {
                ...
                return (
                  <div key={day} onClick={() => dayItems.length ? setSelectedDay(isSelected ? null : day) : null}
                    className={`rounded-[8px] p-0.5 flex flex-col items-center min-h-[52px] transition-colors ${dayItems.length ? 'cursor-pointer active:bg-[#f5f5f7]' : ''} ${isSelected ? 'bg-[#f0f7ff]' : ''}`}>
                    <div className={`w-6 h-6 flex items-center justify-center rounded-full text-[12px] font-medium mb-0.5 ${isToday ? 'bg-[#007aff] text-white' : isPast ? 'text-[#aeaeb2]' : 'text-[#1d1d1f]'}`}>
                      {day}
                    </div>
                    <div className="flex flex-col gap-0.5 w-full">
                      {dayCharges.slice(0, 1).map(c => (
                        <div key={c.id} className={`w-full rounded-[3px] text-[8px] font-semibold px-0.5 leading-[1.4] truncate
                          ${isDone(c.id) ? 'bg-[#e8faf0] text-[#1a7f3c]' : getDayStatus(c) === 'late' ? 'bg-[#fef0f0] text-[#c0392b]' : 'bg-[#fff3e0] text-[#b45309]'}`}>
                          {c.icon} {c.name.slice(0, 4)}
                        </div>
                      ))}
                      {dayIncomes.slice(0, 1).map(c => (
                        <div key={c.id} className={`w-full rounded-[3px] text-[8px] font-semibold px-0.5 leading-[1.4] truncate bg-[#e8f4ff] text-[#185fa5]`}>
                          {c.icon} {c.name.slice(0, 4)}
                        </div>
                      ))}
                      {dayItems.length > 2 && <div className="text-[8px] text-[#86868b] text-center">+{dayItems.length - 2}</div>}
                    </div>
                  </div>
                )
              })}
          ...
          {selectedDay && selectedItems.length > 0 && (
            <div className="bg-white rounded-[20px] overflow-hidden">
              <div className="px-4 py-3 border-b border-[#f2f2f7] flex items-center justify-between">
                ...
                <button onClick={() => setSelectedDay(null)} className="w-6 h-6 rounded-full bg-[#f2f2f7] flex items-center justify-center">
              ...
              {selectedItems.map((item, i) => {
                ...
                return (
                  <div key={item.id} className={`flex items-center px-4 py-3.5 gap-3 ${i < selectedItems.length - 1 ? 'border-b border-[#f2f2f7]' : ''}`}>
                    <div className="w-10 h-10 rounded-[11px] flex items-center justify-center text-xl flex-shrink-0"
                      style={{ background: item.color ?? '#f5f5f7' }}>{item.icon}</div>
                    <div className="flex-1">
                      <p className="text-[15px] font-medium text-[#1d1d1f]">{item.name}</p>
                      <p className={`text-[12px] font-medium ${item.type === 'income' ? 'text-[#34c759]' : done ? 'text-[#34c759]' : status === 'late' ? 'text-[#ff3b30]' : 'text-[#86868b]'}`}>
                        {item.type === 'income' ? '+' : '−'}{fmt(item.amount)}
                      </p>
                    </div>
                    <button onClick={() => toggleDone(item)}
                      className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${done ? 'bg-[#34c759]' : status === 'late' ? 'bg-[#fef0f0] border border-[#ff3b30]' : 'bg-[#f2f2f7] border border-[#d1d1d6]'}`}>
                      <Check size={15} color={done ? 'white' : status === 'late' ? '#ff3b30' : '#aeaeb2'} strokeWidth={2.5} />
                    </button>
                  </div>
                )
              })}
          ...
          <div className="flex bg-white rounded-[16px] p-1 gap-1">
            {(['charge', 'income'] as const).map(t => (
              <button key={t} onClick={() => setTab(t)}
                className={`flex-1 h-9 rounded-[12px] text-[14px] font-semibold transition-all ${tab === t ? 'bg-[#1d1d1f] text-white' : 'text-[#86868b]'}`}>
          ...
          {displayItems.length > 0 ? (
            <div className="bg-white rounded-[20px] overflow-hidden">
              {displayItems.map((item, i) => {
                ...
                const done = isDone(item.id)
                return (
                  <div key={item.id} className={`flex items-center px-4 py-3.5 gap-3 ${i < displayItems.length - 1 ? 'border-b border-[#f2f2f7]' : ''}`}>
                    <div className="w-10 h-10 rounded-[11px] flex items-center justify-center text-xl flex-shrink-0"
                      style={{ background: item.color ?? '#f5f5f7' }}>{item.icon}</div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[15px] font-medium text-[#1d1d1f]">{item.name}</p>
                      <p className={`text-[12px] font-medium ${done ? 'text-[#34c759]' : status === 'late' ? 'text-[#ff3b30]' : 'text-[#86868b]'}`}>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <button onClick={() => toggleDone(item)}
                        className={`w-7 h-7 rounded-full flex items-center justify-center transition-all ${done ? 'bg-[#34c759]' : 'bg-[#f2f2f7] border border-[#d1d1d6]'}`}>
                        <Check size={13} color={done ? 'white' : '#aeaeb2'} strokeWidth={2.5} />
                      </button>
                    </div>
                  </div>
                )
            ...
            <div className="bg-white rounded-[20px] px-4 py-10 text-center">
              ...
              <p className="text-[15px] font-medium text-[#1d1d1f]">Aucun {tab === 'charge' ? 'abonnement' : 'revenu'}</p>
      ...
      <div className="fixed bottom-20 right-4 flex flex-col gap-2 z-40">
        ...
        <button onClick={() => openAdd('charge')}
          className="w-14 h-14 bg-[#1d1d1f] rounded-full flex items-center justify-center shadow-lg active:scale-95 transition-transform"
        ...
        <div className="fixed inset-0 bg-black/30 z-50 flex items-end justify-center"
          ...
          <div className="bg-white rounded-t-[24px] w-full max-w-lg p-5 pb-10">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="text-[13px] text-[#86868b] block mb-1.5">Jour du mois</label>
                {/* Toggle fixe / fin de mois */}
                <div className="flex bg-[#f2f2f7] rounded-[10px] p-0.5 mb-2">
                  <button onClick={() => setFDayMode('fixed')}
                    className={`flex-1 h-8 rounded-[8px] text-[13px] font-medium transition-all ${fDayMode === 'fixed' ? 'bg-white text-[#1d1d1f] shadow-sm' : 'text-[#86868b]'}`}>
                    Jour fixe
                  </button>
                  <button onClick={() => setFDayMode('end')}
                    className={`flex-1 h-8 rounded-[8px] text-[13px] font-medium transition-all ${fDayMode === 'end' ? 'bg-white text-[#1d1d1f] shadow-sm' : 'text-[#86868b]'}`}>
                    Fin du mois − X
                  </button>
                </div>
                {fDayMode === 'fixed' ? (
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[14px] text-[#86868b]">Fin du mois −</span>
                    <input type="number" min="0" max="15"
                      className="w-20 h-11 border border-[#d1d1d6] rounded-[12px] px-3.5 text-[15px] bg-[#f9f9fa] outline-none focus:border-[#007aff] focus:bg-white"
                      placeholder="2" value={fDay} onChange={e => setFDay(e.target.value)} />
                    <span className="text-[14px] text-[#86868b]">jours</span>
                  </div>
                )}
                {fDay && (
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {CATEGORIES.map(cat => (
                      <button key={cat.id} onClick={() => setFCategory(cat.id)}
                        className={`h-10 rounded-[10px] flex items-center justify-center gap-1.5 text-[12px] font-medium transition-all border ${fCategory === cat.id ? 'border-[#007aff] bg-[#e8f0ff] text-[#007aff]' : 'border-[#d1d1d6] bg-[#f9f9fa] text-[#86868b]'}`}>
                        <span>{cat.icon}</span>
